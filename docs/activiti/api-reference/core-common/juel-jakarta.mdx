...
```mermaid
flowchart TD
    subgraph JUEL["JUEL Implementation"]
        EFI["ExpressionFactoryImpl<br/>(Jakarta ExpressionFactory)<br/>- createValueExpression()<br/>- createMethodExpression()<br/>- Expression caching<br/>- Profile configuration"]
        TB["TreeBuilder<br/>(Expression Parser)<br/>- Parse expression string<br/>- Build AST tree<br/>- Support method invocations<br/>- Support varargs"]
        subgraph AST["AST Tree"]
            AB["AstBinary (operators)"]
            AF["AstFunction (functions)"]
            AM["AstMethod (method calls)"]
            AP["AstProperty (property access)"]
            AI["AstIdentifier (variables)"]
            AL["AstLiteral (literals)"]
        end
        TVE["TreeValueExpression<br/>(Expression Evaluation)<br/>- getValue(ELContext)<br/>- setValue(ELContext, Object)<br/>- Type conversion"]
        EFI -->|uses| TB
        TB -->|creates| AST
        AST -->|evaluated by| TVE
    end
```
...
flowchart TD
    ...
    EFI["ExpressionFactoryImpl<br/>createValueExpression()"]
    TB["TreeBuilder<br/>Parse expression"]
    AST["AST Tree<br/>AstBinary(&gt;)<br/>AstProperty(totalAmount)<br/>AstNumber(1000)"]
    TVE["TreeValueExpression<br/>Evaluate tree"]
    RES["Result: true/false"]
    ES --> EXPR --> EFI --> TB --> AST --> TVE --> RES
```
...
Double value = converter.convert("123.45", Double.class);
```
...
public void validateExpression(String expression) {
    try {
        ...
        // No custom exception type needed — jakarta.el.ELException is the standard EL error type
...
Object result = expr.getValue(context);
if (result != null) {
    Double amount = typeConverter.convert(result, Double.class);
}
```
