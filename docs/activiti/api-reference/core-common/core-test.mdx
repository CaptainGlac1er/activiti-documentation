...
flowchart TD
    ...
    subgraph Operations["Operations API"]
        OA["ProcessOperations.start(payload),<br>TaskOperations.claim(payload)"]
    end

    subgraph Assertions["Assertion Wrappers<br>(ProcessInstanceAssertions, TaskAssertions, SignalAssertions)"]
        AW["expectFields(matchers...) → ProcessResultMatcher / TaskResultMatcher<br>expectEvents(matchers...) → OperationScopeMatcher<br>expect(matchers...) → ProcessTaskMatcher<br>andReturn() → underlying domain object"]
    end

    subgraph Runtime["Local Runtime"]
        LR["ProcessRuntime, TaskRuntime, EventSource, TaskSource"]
    end
...
ProcessInstance instance = processOperations
    ...
    .expectFields(
        ...
        processInstance().businessKey("ORD-123")
    )
    .expectEvents(
...
Task claimedTask = taskOperations
    .claim(TaskPayloadBuilder.claim().withTaskId(taskId)
...
processOperations
    .signal(ProcessPayloadBuilder.signal().withName("ORDER_APPROVED")
        .build())
...
Task completedTask = taskOperations
    .complete(TaskPayloadBuilder.complete().withTaskId(taskId)
...
class TaskTest {
    ...
    void testTaskClaimAndComplete() {
        ...
        Task task = page.getContent().get(0);

        // Claim and assert
        Task claimedTask = taskOperations
            .claim(TaskPayloadBuilder.claim().withTaskId(task.getId())
                .withAssignee("john.doe")
                .build())
            .expectFields(
                withAssignee("john.doe")
        ...
        // Complete and assert
        Task completedTask = taskOperations
            .complete(TaskPayloadBuilder.complete().withTaskId(task.getId())
                .withVariable("approved", true)
                .build())
            .expectEvents(
...
class ProcessInstanceMatcherTest {
    ...
    void testProcessInstanceMatchers() {
        processOperations
            ...
            .expect(
                processInstance().hasTask("Review Order", Task.TaskStatus.CREATED)
            );
    }
}
```
...
processOperations
    .signal(ProcessPayloadBuilder.signal().withName("PROCEED")
        .build())
    .expectEventsOnProcessInstance(instance,
...
class OrderProcessIntegrationTest {
    ...
    void testCompleteOrderProcess() {
        ...
        Task reviewTask = page.getContent().get(0);

        // 3. Claim task and assert
        taskOperations
            .claim(TaskPayloadBuilder.claim().withTaskId(reviewTask.getId())
        ...
        taskOperations
            .complete(TaskPayloadBuilder.complete().withTaskId(reviewTask.getId())
                .withVariable("approved", true)
                .build())
            .expectEvents(task().hasBeenCompleted());
