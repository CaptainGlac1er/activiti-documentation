...
<DirectoryTree rootName="activiti-spring-connector" expandAll dotWalk showFirst={5}>
              ...
              <DirectoryTree.Dir name="common">
                <DirectoryTree.Dir name="spring">
                  <DirectoryTree.Dir name="connector">
                    <DirectoryTree.File name="ConnectorDefinitionService.java" />
                    <DirectoryTree.Dir name="autoconfigure">
                      <DirectoryTree.File name="ConnectorAutoConfiguration.java" />
                    </DirectoryTree.Dir>
                  </DirectoryTree.Dir>
                </DirectoryTree.Dir>
              </DirectoryTree.Dir>
...
```mermaid
flowchart TD
    subgraph CM["Connector Management"]
        CDS["ConnectorDefinitionService<br/>Responsibilities:<br/>- Load connector JSON files<br/>- Parse ConnectorDefinition objects<br/>- Validate connector definitions<br/>- Ensure unique names"]
        RPR["Resource Pattern Resolver"]
        JO["Jackson JsonMapper"]
        VL["Validation Logic"]
        CD["ConnectorDefinition<br/>(From activiti-connector-model)<br/>- id, name, description<br/>- actions (map of ActionDefinition)"]
        CDS --> RPR
        CDS --> JO
        CDS --> VL
    end
...
flowchart TD
    ...
    CAC["ConnectorAutoConfiguration<br/>Creates service bean"]
    CDS["ConnectorDefinitionService<br/>get() method"]
    RPR["ResourcePatternResolver<br/>Find *.json files<br/>in connector root"]
    EACH["For each JSON file:<br/>- Read InputStream<br/>- Parse with JsonMapper<br/>- Create ConnectorDef"]
    VAL["Validate connectors<br/>- Check names not null<br/>- No dots in names<br/>- Unique names"]
    RET["Return list of<br/>ConnectorDefinitions"]
    AS --> CAC --> CDS --> RPR --> EACH --> VAL --> RET
```
...
**Dependencies:**
- `JsonMapper` (Jackson 3, `tools.jackson.databind.json.JsonMapper`) - JSON parsing
...
- `JsonMapper` (Jackson 3) is injected from the Spring context (no `ObjectMapper` bean is created)
...
> **Note:** Variable mapping for connectors is configured via **process extension JSON** (`*-extensions.json`), not through `<activiti:in>` or `<activiti:out>` XML elements. The engine does not parse these elements.

**notificationProcess-extensions.json:**
```json
{
  "extensions": {
    "notificationProcess": {
      "mappings": {
        "sendEmail": {
          "inputs": {
            "to": { "type": "VARIABLE", "value": "recipientEmail" },
            "subject": { "type": "EXPRESSION", "value": "${'Order #' + orderId}" },
            "body": { "type": "VARIABLE", "value": "emailBody" }
          },
          "outputs": {
            "sentMessageId": { "type": "VARIABLE", "value": "messageId" }
          }
        }
      }
    }
...
**Problem:** JsonMapper fails to parse JSON
...
- [Connector Model](./connector-model.mdx)
