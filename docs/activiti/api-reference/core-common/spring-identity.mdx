...
<DirectoryTree rootName="activiti-spring-identity" expandAll dotWalk showFirst={5}>
              ...
              <DirectoryTree.Dir name="common">
                <DirectoryTree.Dir name="spring">
                  <DirectoryTree.Dir name="identity">
                    <DirectoryTree.File name="ActivitiUserGroupManagerImpl.java" />
                    <DirectoryTree.File name="ExtendedInMemoryUserDetailsManager.java" />
                    <DirectoryTree.Dir name="config">
                      <DirectoryTree.File name="ActivitiSpringIdentityAutoConfiguration.java" />
                    </DirectoryTree.Dir>
                  </DirectoryTree.Dir>
                </DirectoryTree.Dir>
              </DirectoryTree.Dir>
...
classDiagram
    ...
    class ActivitiUserGroupManagerImpl {
    ...
    class ExtendedInMemoryUserDetailsManager {
        In-memory store for testing/development
        +getUsers()
        +getGroups()
    }
    UserGroupManager <|-- ActivitiUserGroupManagerImpl : implemented by
    ActivitiUserGroupManagerImpl --> UserDetailsService : uses
    UserDetailsService <|-- ExtendedInMemoryUserDetailsManager : implemented by
```
...
flowchart TD
    UL["User Login"]
    SSA["Spring Security Authentication"]
    UDS["UserDetailsService<br/>loadUserByUsername()"]
    UD["UserDetails with GrantedAuthorities<br/>- ROLE_ADMIN<br/>- GROUP_HR"]
    UGM["ActivitiUserGroupManager<br/>getUserGroups(username)"]
    EX["Extract GROUP_ authorities<br/>→ [#quot;HR#quot;]"]
    UL --> SSA
    SSA --> UDS
    UDS --> UD
    UD --> UGM
...
- [Spring Security Module](./spring-security.mdx)
