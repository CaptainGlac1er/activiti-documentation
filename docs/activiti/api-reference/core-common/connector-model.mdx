...
<DirectoryTree rootName="activiti-connector-model" expandAll dotWalk showFirst={5}>
...
classDiagram
    ...
    class ActionDefinition {
        ...
        +String name
        +String description
        +List~VariableDefinition~ inputs
        +List~VariableDefinition~ outputs
    }
    ...
    ConnectorDefinition --> ActionDefinition : contains
    ActionDefinition --> VariableDefinition : has
```
...
### Email Connector Variable Mapping
  ...
  "extensions": {
    "orderProcess": {
      "mappings": {
        "sendNotification": {
          "inputs": {
            "to": { "type": "VARIABLE", "value": "order.customerEmail" },
            "subject": { "type": "EXPRESSION", "value": "${'Order #' + orderId}" },
            "body": { "type": "VARIABLE", "value": "orderDetails" },
            "isHtml": { "type": "VALUE", "value": true }
          },
          "outputs": {
            "emailMessageId": { "type": "VARIABLE", "value": "messageId" }
          }
        }
      }
    }
