...
title: "Spring Security Policies"
...
<DirectoryTree rootName="activiti-spring-security-policies" expandAll dotWalk showFirst={5}>
...
classDiagram
    ...
    class ProcessSecurityPoliciesManager {
    ...
    class RestrictionApplier~T~ {
        <<interface>>
    }
    class SecurityPoliciesProcessDefinitionRestrictionApplier {
        implements RestrictionApplier~T~
    }
    class SecurityPoliciesProcessInstanceRestrictionApplier {
        implements RestrictionApplier~T~
    }
    class SecurityPolicy {
        ...
        +List~String~ users
        +String serviceName
        +SecurityPolicyAccess access
        +List~String~ keys
    }
    SecurityPoliciesManager <|-- ProcessSecurityPoliciesManager : extended by
    ProcessSecurityPoliciesManager <|-- ProcessSecurityPoliciesManagerImpl : implemented by
    BaseSecurityPoliciesManagerImpl <|-- ProcessSecurityPoliciesManagerImpl : extends
    ProcessSecurityPoliciesManagerImpl --> SecurityPoliciesProcessDefinitionRestrictionApplier : uses
    ProcessSecurityPoliciesManagerImpl --> SecurityPoliciesProcessInstanceRestrictionApplier : uses
    RestrictionApplier~T~ <|.. SecurityPoliciesProcessDefinitionRestrictionApplier : implements
    RestrictionApplier~T~ <|.. SecurityPoliciesProcessInstanceRestrictionApplier : implements
```
...
flowchart TD
    PQ["Process Query"]
    PSM["ProcessSecurityPoliciesManager"]
    RQ["restrictProcessDefQuery<br/>or<br/>restrictProcessInstQuery<br/>(SecurityPolicyAccess)"]
    PD{"Are policies defined?"}
    EVAL["evaluate"]
    GAK["getAllowedKeys()<br/>- Check user in policy<br/>- Check groups in policy<br/>- Match access level<br/>- Collect allowed keys"]
    A1["allowAll()"]
    A2["allowAll()"]
    RK["restrictTo(keys)"]
    DA["denyAll()"]
    ...
    PD -->|Yes| EVAL
    EVAL --> GAK
    GAK -->|Wildcard found| A2
    GAK -->|Keys found| RK
    GAK -->|No keys match| DA
```
...
```mermaid
flowchart TD
    RQ["restrictQuery(applier, access)"]
    B1["arePoliciesDefined() == false"]
    B2["keys = definitionKeysAllowedForApplicationPolicy(access)"]
    B3["policies list is empty"]
    RQ --> B1
    RQ --> B2
    RQ --> B3
    B1 --> A1["applier.allowAll()"]
    B2 --> B2a["keys contains wildcard (*)"]
    B2 --> B2b["keys not empty"]
    B2 --> B2c["keys empty AND policies exist"]
    B2a --> A2["applier.allowAll()"]
    B2b --> A3["applier.restrictToKeys(keys)"]
    B2c --> A4["applier.denyAll()"]
    B3 --> A5["applier.allowAll()"]
```
...
public class ProcessQueryService {
    ...
    public List<ProcessDefinition> getVisibleProcessDefinitions() {
        ...
        return processRuntime.processDefinitions(Pageable.ofSize(25), payload).getContent();
    }
    ...
    public List<ProcessInstance> getVisibleProcessInstances() {
        ...
        return processRuntime.processInstances(Pageable.ofSize(25), payload).getContent();
    }
}
```
