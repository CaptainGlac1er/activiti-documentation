...
<DirectoryTree rootName="activiti-expression-language" expandAll dotWalk showFirst={5}>
          ...
          <DirectoryTree.Dir name="activiti">
            <DirectoryTree.Dir name="core">
              <DirectoryTree.Dir name="el">
                <DirectoryTree.File name="ActivitiElContext.java" />
                <DirectoryTree.File name="ELContextBuilder.java" />
                <DirectoryTree.File name="ExpressionResolver.java" />
                <DirectoryTree.File name="JuelExpressionResolver.java" />
                <DirectoryTree.File name="ReadOnlyMapELResolver.java" />
                <DirectoryTree.File name="JsonNodeELResolver.java" />
                <DirectoryTree.File name="CommonELResolversUtil.java" />
                <DirectoryTree.File name="DateResolverHelper.java" />
                <DirectoryTree.File name="ListResolverHelper.java" />
                <DirectoryTree.File name="ELResolverReflectionBlockerDecorator.java" />
              </DirectoryTree.Dir>
            </DirectoryTree.Dir>
          </DirectoryTree.Dir>
        </DirectoryTree.Dir>
...
```mermaid
flowchart TD
    subgraph EL["Expression Language"]
        AEC["ActivitiElContext<br/>(Jakarta EL Context Implementation)<br/>- ELResolver: Property resolution<br/>- FunctionMapper: Custom functions<br/>- VariableMapper: Variable access"]
        ELR["ELResolver<br/>Property access"]
        FM["Function Mapper<br/>Activiti functions"]
        VM["Variable Mapper<br/>Activiti variables"]
        AEC --> ELR
        AEC --> FM
        AEC --> VM
    end
...
flowchart TD
    BE["BPMN Expression"]
    EXPR["${order.totalAmount &gt; 1000}"]
    EF["ExpressionFactory<br/>getExpression()"]
    AEC["ActivitiElContext<br/>evaluate()"]
    VM["Variable Mapper"]
    FM["Function Mapper"]
    PR["Property Resolver"]
    RES["Result: true"]
    BE --> EXPR --> EF --> AEC
    AEC --> VM
    AEC --> FM
...
### Custom Expression Factory (SPI)

:::info[Added in 9.0.0]
The engine's internal `ExpressionManager` now resolves its `ExpressionFactory` through the standard `jakarta.el.ExpressionFactory.newInstance()` SPI instead of instantiating `ExpressionFactoryImpl` directly. This means you can replace the default JUEL implementation with a custom EL engine by providing a `META-INF/services/jakarta.el.ExpressionFactory` entry on the classpath — no engine modifications required. Default behavior is unchanged.
:::

To supply a custom implementation, register it via the Java ServiceLoader mechanism:

```java
// META-INF/services/jakarta.el.ExpressionFactory
// (one line, the fully-qualified class name of your factory)
com.example.MyCustomExpressionFactory
```

```java
public class MyCustomExpressionFactory extends jakarta.el.ExpressionFactory {
    // Override createValueExpression, createMethodExpression, etc.
    // as needed for your EL implementation.
}
```

The engine picks up the factory automatically at startup. No configuration property or Spring bean is needed — the standard `ServiceLoader` lookup in `ExpressionFactory.newInstance()` finds it.
...
- `getValue()` converts JSON arrays to `List` via `JsonMapper.convertValue()` (Jackson 3, `tools.jackson.databind.json.JsonMapper`)
...
- Expressions use Jakarta EL syntax: `${...}`
...
try {
    ...
    // No custom exception type needed — rethrow as jakarta.el.ELException with context
    throw new ELException("Invalid expression: " + expression, e);
}
...
- [Testing Framework](./core-test.mdx)
