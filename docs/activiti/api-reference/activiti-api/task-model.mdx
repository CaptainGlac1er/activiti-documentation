...
<DirectoryTree rootName="activiti-api-task-model" expandAll dotWalk showFirst={5}>
              ...
              <DirectoryTree.Dir name="task">
                <DirectoryTree.Dir name="model">
                  <DirectoryTree.Dir name="events">
                    <DirectoryTree.File name="TaskRuntimeEvent.java" />
                    <DirectoryTree.File name="TaskCandidateUserEvent.java" />
                    <DirectoryTree.File name="TaskCandidateGroupEvent.java" />
                  </DirectoryTree.Dir>
                  <DirectoryTree.Dir name="results">
                    <DirectoryTree.File name="TaskResult.java" />
                  </DirectoryTree.Dir>
                  <DirectoryTree.Dir name="builders">
                    <DirectoryTree.File name="TaskPayloadBuilder.java" />
                    <DirectoryTree.File name="CompleteTaskPayloadBuilder.java" />
                    <DirectoryTree.File name="CreateTaskPayloadBuilder.java" />
                    <DirectoryTree.File icon="…" name="Other Builders" />
                  </DirectoryTree.Dir>
                  <DirectoryTree.Dir name="payloads">
                    <DirectoryTree.File name="CompleteTaskPayload.java" />
                    <DirectoryTree.File name="CreateTaskPayload.java" />
                    <DirectoryTree.File name="ClaimTaskPayload.java" />
                    <DirectoryTree.File icon="…" name="Other Payloads" />
                  </DirectoryTree.Dir>
                  <DirectoryTree.File name="Task.java" />
                  <DirectoryTree.File name="TaskCandidate.java" />
                  <DirectoryTree.File name="TaskCandidateUser.java" />
                  <DirectoryTree.File name="TaskCandidateGroup.java" />
                </DirectoryTree.Dir>
                <DirectoryTree.Dir name="runtime">
                  <DirectoryTree.Dir name="events">
                    <DirectoryTree.File name="TaskCreatedEvent.java" />
                    <DirectoryTree.File name="TaskCompletedEvent.java" />
                    <DirectoryTree.File icon="…" name="Other Runtime Events" />
                  </DirectoryTree.Dir>
                </DirectoryTree.Dir>
              </DirectoryTree.Dir>
            </DirectoryTree.Dir>
...
**Triggered When**: A task's suspension state changes to active. When the containing process instance is activated, the engine cascades the new suspension state to all of the instance's tasks, firing this event per task.

**Payload**: The current task state via `getEntity()` (`Task`), plus the standard runtime event fields — no additional event-specific fields.
...
**Triggered When**: A task's suspension state changes to suspended. When the containing process instance is suspended, the engine cascades the new suspension state to all of the instance's tasks, firing this event per task.
...
**Triggered When**: A task's persisted state is updated — a generic task change such as a name, description, priority, or due date update. Assignment/claim also fires this event, since the engine dispatches the assignment event alongside the underlying update event.
...
**Usage**:
```java
Task completedTask = taskRuntime.complete(payload);
```
...
```mermaid
flowchart TD
    A["Task Created (CREATED)"] --> B["Candidate Users & Groups Set"]
    B --> C["User Claims (ASSIGNED)"]
    C --> D["Task Work In Progress"]
    D --> E["Task Completed (COMPLETED)"]
```
...
void taskShouldTrackCandidateChanges() {
    ...
    assertTrue(taskRuntime.task(taskId).getCandidateUsers().contains("user1"));
    ...
    assertFalse(taskRuntime.task(taskId).getCandidateUsers().contains("user1"));
}
```
...
```java
@Test
void completeTaskShouldThrowWhenTaskNotFound() {
    assertThrows(NotFoundException.class, () -> {
        taskRuntime.complete(
            TaskPayloadBuilder.complete()
                .withTaskId("non-existing-task")
                .build()
        );
    });
}
...
```java
// No mutators on the Task model - candidates are managed via the runtime API

// Good - adding specific candidates
taskRuntime.addCandidateUsers(payload);

// Removing specific candidates
taskRuntime.deleteCandidateUsers(payload);
```
...
- [Process Model Module](./process-model.mdx)
