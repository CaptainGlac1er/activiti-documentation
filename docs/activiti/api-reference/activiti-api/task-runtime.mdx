...
:::info[Added in 8.7.0]
The admin-only query `lastCreatedTaskByProcessInstanceIdAndTaskDefinitionKey(String, String)` was added to `TaskAdminRuntime` in version 8.7.0.
:::
...
public interface TaskAdminRuntime {
    ...
    // Admin-only query (Added in 8.7.0)
...
- User must be the task owner (no admin bypass in user runtime; admin access is via `TaskAdminRuntime`)
...
**Default Query**: Always scoped to the authenticated user. Returns tasks where:
- User is the assignee, OR
- User is a candidate user or is in a candidate group, OR
...
**Note**: `assignee` and `groups` values set on the payload are not applied as filters. The user runtime overwrites `assigneeId`/`groups` with the authenticated user and the user's groups before building the query, so filtering by another user or group is silently ignored. The admin runtime (`TaskAdminRuntime.tasks`) ignores `assigneeId`/`groups` entirely and applies only `processInstanceId` and `parentTaskId`.
...
Task task = taskRuntime.create(
    TaskPayloadBuilder.create()
        ...
        .withCandidateUsers("backup.reviewer")
...
**Authorization**:
- User must be able to see the task
- User must be the assignee or the task owner
...
└─ User is the task owner
(No admin bypass in user runtime; admin access is via TaskAdminRuntime)
```
...
```mermaid
flowchart TD
    A["Query Task"] --> B["Check Visibility Rules"]
    B -->|YES| C["Task"]
    B -->|NO| D["NotFound"]
```
...
Use specific filters (`processInstanceId` and `parentTaskId` are the effective filters; `assignee`/`groups` are scoped to the authenticated user):
```java
// Good - specific query
...
Delete: Must be assignee or owner
```
...
- [Runtime Events](./runtime-events.md) — the event system that backs these operations
