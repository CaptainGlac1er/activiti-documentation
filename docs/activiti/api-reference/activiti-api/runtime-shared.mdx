...
<DirectoryTree rootName="activiti-api-runtime-shared" expandAll dotWalk showFirst={5}>
...
```mermaid
flowchart TD
    A["Request"] --> B["SecurityManager"]
    B --> C["Principal Providers"]
    B --> D["Authorization Decision"]
```
...
// AbstractSecurityManager is abstract - use a concrete subclass
@Component
public class SpringSecurityManager extends AbstractSecurityManager {
    public SpringSecurityManager() {
        super(
            ...
            new PrincipalGroupsProvider() {
                @Override
                public List<String> getGroups(Principal principal) {
                    return List.of();
                }
            },
            new PrincipalRolesProvider() {
                @Override
