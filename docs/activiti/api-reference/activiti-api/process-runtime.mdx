...
<DirectoryTree rootName="activiti-api-process-runtime" expandAll dotWalk showFirst={5}>
              ...
              <DirectoryTree.Dir name="process">
                <DirectoryTree.Dir name="runtime">
                  <DirectoryTree.Dir name="conf">
                    <DirectoryTree.File name="ProcessRuntimeConfiguration.java" />
                  </DirectoryTree.Dir>
                  <DirectoryTree.Dir name="events">
                    <DirectoryTree.Dir name="listener">
                      <DirectoryTree.File name="ProcessRuntimeEventListener.java" />
                      <DirectoryTree.File name="ProcessEventListener.java" />
                      <DirectoryTree.File name="BPMNElementEventListener.java" />
                      <DirectoryTree.File name="ProcessCandidateStarterEventListener.java" />
                    </DirectoryTree.Dir>
                  </DirectoryTree.Dir>
                  <DirectoryTree.Dir name="connector">
                    <DirectoryTree.File name="Connector.java" />
                  </DirectoryTree.Dir>
                  <DirectoryTree.File name="ProcessRuntime.java" />
                  <DirectoryTree.File name="ProcessAdminRuntime.java" />
                </DirectoryTree.Dir>
              </DirectoryTree.Dir>
            </DirectoryTree.Dir>
          </DirectoryTree.Dir>
...
:::info[Added in 8.8.0]
`GetProcessDefinitionsPayload` supports multiple process definition IDs in a single call, and a new API retrieves the latest versions of process definitions. The process definition query can also include non-user-startable processes (previously only user-startable ones were returned by default).
:::
...
**Authorization**:
- Security policy must allow read access to the process definition
- Checks if user is the initiator OR an assignee/candidate of a task in the instance
- No admin bypass in user runtime; admin access is via `ProcessAdminRuntime`
...
**Authorization**:
- Security policy must allow write access to the process definition
- User must be the process initiator (`startUserId`)
- Being a task assignee or candidate of the instance grants no write access
- No admin bypass in user runtime; admin access is via `ProcessAdminRuntime`
...
- Name
- Business key
- Description — present in the payload but not applied by the runtime
...
**Returns**:
- `ProcessDefinitionMeta` exposing only `getProcessDefinitionKey()` (read access is checked via `processDefinition(...)`)
...
**Returns**:
- `ProcessInstanceMeta` exposing `getProcessInstanceId()` and `getActiveActivitiesIds()` (read access is checked via `processInstance(...)`)
...
```java
@Component
public class AsyncProcessCompletionListener
...
**Visibility Rules**:
```java
User can read a process instance if:
- Security policy allows read (canRead) on the process definition AND
- User is the initiator OR
- User is an assignee or candidate of a task in the process
(No admin bypass in user runtime; admin access is via ProcessAdminRuntime)
```
...
```java
@Component
public class IdempotentProcessCompletionListener
    implements ProcessRuntimeEventListener<ProcessCompletedEvent> {
