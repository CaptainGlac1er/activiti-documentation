...
4. **Hierarchy**
   ...
   - `rootProcessInstanceId`: Root process instance ID in a call-activity hierarchy *(Added in 8.8.0)*
...
**Purpose**: Payload modeling the message metadata of a BPMN message event — it is the payload carried by the `BPMNMessage` entity (`getMessagePayload()`). It is **not** an input to `ProcessRuntime`: to send a message to a running process, use `ReceiveMessagePayload` with `ProcessRuntime.receive(...)`; to start a process by message, use `StartMessagePayload` with `ProcessRuntime.start(StartMessagePayload)`. Neither of those is a subclass of `MessageEventPayload`.

**Key Fields**:

1. **Message Identification**
   - `name`: BPMN message name
...
**Usage Example** (reading the payload from a message event entity, e.g. `BPMNMessageReceivedEvent`):
```java
BPMNMessage message = event.getEntity();
MessageEventPayload payload = message.getMessagePayload();

String messageName = payload.getName();
String correlationKey = payload.getCorrelationKey();
```
...
**Usage Example**:
```java
SignalPayload payload = ProcessPayloadBuilder.signal()
...
processRuntime.removeVariables(
    ProcessPayloadBuilder.removeVariables()
        .withProcessInstanceId(instanceId)
        .withVariableNames("tempVar")
        .withVariableNames("oldData")
        .build()
);
```
...
:::info[Added in 8.8.0]
Multiple process definition IDs and a `latestVersionOnly` option were added to `GetProcessDefinitionsPayload` in version 8.8.0, enabling batch lookups and filtering for the latest version of each process.
:::
...
// Model a BPMN message event payload (BPMNMessage entity)
MessageEventPayload
```
...
@Test
void startProcessShouldThrowForUnknownProcessDefinition() {
    assertThrows(ActivitiObjectNotFoundException.class, () -> {
        processRuntime.start(
            ProcessPayloadBuilder.start()
                .withProcessDefinitionKey("nonExistent") // missing/unknown process definition
                .build()
        );
    });
}
