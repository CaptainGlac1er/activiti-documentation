...
slug: /api-reference/engine-api/spring-boot-starter
...
<DirectoryTree rootName="activiti-spring-boot-starter" expandAll dotWalk showFirst={5}>
...
flowchart TD
    subgraph SpringBootApp["Spring Boot Application"]
        ...
        subgraph SecurityCfg["ActivitiMethodSecurityAutoConfiguration"]
            S1["@ConditionalOnClass<br>(EnableMethodSecurity)"]
        end
    end
...
- `EnableMethodSecurity` is on the classpath
- No existing bean annotated with `@EnableMethodSecurity` is defined
...
spring:
  activiti:
    ...
    # Disables start event subscriptions of previous deployments when a new
    # version is deployed (prevents accidental triggering of superseded versions).
    # Default: false. Added in 8.8.0.
    disable-existing-start-event-subscriptions: false
...
Process extension JSON files are discovered by the `ProcessExtensionResourceFinderDescriptor` bean using dedicated properties:

| Property | Default | Description |
|----------|---------|-------------|
| `spring.activiti.process.extensions.dir` | `NOT_DEFINED` — falls back to `spring.activiti.process-definition-location-prefix` | Location prefix for scanning process extension files |
| `spring.activiti.process.extensions.suffix` | `**-extensions.json` | Single file-suffix string; only files ending in `-extensions.json` are loaded |
| `spring.activiti.applicationsLocation` | `classpath:/applications/` | String location scanned for Activiti application archives (`.zip`) that are auto-deployed at startup. See [Spring Application](../core-common/spring-application.md) |

`spring.activiti.process-definition-location-suffixes` (the BPMN suffix list) is **not** used for extension scanning.
...
spring:
  activiti:
    async-executor:
      # Retry wait time for failed jobs. Despite the name, applied as SECONDS. Default: 500
...
public class CustomProcessDeployedListener
    ...
    public void onEvent(ProcessDeployedEvent event) {
        log.info("Process deployed: {}", event.getEntity().getKey());
    }
}
```
...
> **Note:** `CustomRejectedJobsHandler` implements `SpringRejectedJobsHandler`, which is `@Deprecated @Internal` in source — an internal, deprecated API that may change or be removed. See [Spring Integration — SpringRejectedJobsHandler](./spring-integration.mdx#springrejectedjobshandler).
...
Method security is **enabled by default** — `ActivitiMethodSecurityAutoConfiguration` is
conditional on the property with `@ConditionalOnProperty(name = "spring.activiti.security.enabled", matchIfMissing = true)`.
Setting the property to `true` is therefore a no-op; the property only **disables** method
security when set to `false`.

- Check that `spring.activiti.security.enabled` has not been set to `false` in your configuration
  (remove the property or set it back to `true` to restore the default behavior).
- Ensure `EnableMethodSecurity` is on the classpath — the auto-configuration is
  also conditional on this class being present (`@ConditionalOnClass`).
...
| `ActivitiMethodSecurityAutoConfiguration` | `org.activiti.spring.boot` | Enables method-level security when `EnableMethodSecurity` is on classpath. |
