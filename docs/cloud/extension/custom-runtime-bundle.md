...
| `spring.activiti.deployment-mode` | `never-fail` (set by the starter; the engine's own default is `default`) | Auto-deployment strategy. With `never-fail`, a model that fails validation is skipped with an error log instead of failing startup. Other supported modes: `default`, `fail-on-no-process`, `single-resource`, `resource-parent-folder`, and `none` (no auto-deployment at all — no resources are deployed automatically; deploy processes explicitly via the engine API, see [Spring Auto-Deployment Modes](../../activiti/advanced/auto-deployment-modes.md)). |
