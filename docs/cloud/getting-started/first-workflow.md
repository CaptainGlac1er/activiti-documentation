...
Use a user that has the `ACTIVITI_USER` role. The acceptance test realm contains users such as `testuser` and `hruser`; their credentials are configured per environment — the install script generates a `.env` file that points to a separate `.env.local` file where you set `TESTUSER_USERNAME` / `TESTUSER_PASSWORD` (and `HRUSER_*`) — see [Local Development Setup](local-setup.md).
...
```bash
curl -s "$GATEWAY/audit/v1/events?search=processInstanceId:$PROCESS_INSTANCE_ID," \
  -H "Authorization: Bearer $TOKEN"
```

(The `search` parameter is a field-expression filter — see [The `search` filter expression](../services/audit.md#the-search-filter-expression) — not a free-text match.)
