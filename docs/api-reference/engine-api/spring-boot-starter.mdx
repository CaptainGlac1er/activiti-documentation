...
flowchart TD
    subgraph SpringBootApp["Spring Boot Application"]
        ...
        subgraph AsyncProps["AsyncExecutorProperties"]
            P2["@ConfigurationProperties<br>(activiti.async-executor)"]
        end
...
Process extension JSON files are discovered alongside process definitions using `process-definition-location-prefix` and `process-definition-location-suffixes`.

---

## Async Executor Properties

`AsyncExecutorProperties` uses the prefix **`activiti.async-executor`**:
...
| `AsyncExecutorProperties` | `async-executor` | Async job executor: pool sizes, queue, retries, timeouts, acquisition settings. |
