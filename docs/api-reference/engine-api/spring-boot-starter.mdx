...
flowchart TD
    subgraph SpringBootApp["Spring Boot Application"]
        ...
        subgraph SecurityCfg["ActivitiMethodSecurityAutoConfiguration"]
            S1["@ConditionalOnClass<br>(GlobalMethodSecurityConfiguration)"]
        end
    end
...
- Ensure `GlobalMethodSecurityConfiguration` is on the classpath — the auto-configuration is
...
| `ActivitiMethodSecurityAutoConfiguration` | `org.activiti.spring.boot` | Enables method-level security when `GlobalMethodSecurityConfiguration` is on classpath. |
