...
> **Note:** Variable mapping for connectors is configured via **process extension JSON** (`*-extension.json`), not through `<activiti:in>` or `<activiti:out>` XML elements. The engine does not parse these elements.
